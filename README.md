...
it, err = sf.QueryBulkIterator(context.Background(), "SELECT Id, Alternate_Owner__r.Name FROM Contact")
if err != nil {
    panic(err)
}
...
```go
jobIds, err := sf.InsertBulkFileAssign(context.Background(), "Lead", "data/avengers.csv", 1000, false, "01QDn00000112FHMAY")
```
...
jobIds, err := sf.UpdateBulk(context.Background(), "Contact", contacts, 1000, false)
```
...
jobIds, err := sf.UpsertBulk(context.Background(), "Contact", "ContactExternalId__c", contacts, 1000, false)
```
...
for _, id := range jobIds {
    results, err := sf.GetJobResults(context.Background(), id) // returns an instance of BulkJobResults
    if err != nil {
        panic(err)
    }
    fmt.Println(results)
...
resp, err := sf.DoRequest(context.Background(), "GET", "/sobjects/Account/describe", nil,
...
resp, err := sf.DoRequest(context.Background(), "GET", "/sobjects", nil,
    salesforce.WithHeader("If-Modified-Since", "Wed, 21 Oct 2015 07:28:00 GMT"),
